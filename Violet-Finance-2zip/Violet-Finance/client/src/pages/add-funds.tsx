...
const INITIAL: FormData = { userId: "", fullName: "", utr: "", receipt: null, remarks: "" };
...
export default function AddFunds() {
  ...
  const handleSubmit = async () => {
    if (!form.userId.trim()) { toast({ title: "Required", description: "Please enter your Yek7Pay User ID.", variant: "destructive" }); return; }
    if (!form.fullName.trim()) { toast({ title: "Required", description: "Please enter your full name.", variant: "destructive" }); return; }
    if (!form.utr.trim()) { toast({ title: "Required", description: "Please enter the UTR / Bank Reference number.", variant: "destructive" }); return; }
    ...
    try {
      const res = await fetch("/api/fund-requests", {
        ...
        body: JSON.stringify({
          yek7payUserId: form.userId.trim(),
          ...
          {submitted ? (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <div>
                ...
                <Input value={form.userId} onChange={(e) => set("userId", e.target.value)} placeholder="e.g. YEK-00123"
              ...
              <div>
                ...
                <Input value={form.fullName} onChange={(e) => set("fullName", e.target.value)} placeholder="As per bank account"
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/25 rounded-xl h-11 focus:border-emerald-500/50 focus:ring-0" />
              </div>
              ...
              <div>
                ...
                <Input value={form.utr} onChange={(e) => set("utr", e.target.value)} placeholder="12-digit UTR or reference number"
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/25 rounded-xl h-11 focus:border-emerald-500/50 focus:ring-0" />
              </div>
              ...
              <div>
                ...
                <Textarea value={form.remarks} onChange={(e) => set("remarks", e.target.value)}
                  placeholder="e.g. SBI Main Branch, Mumbai — or any additional info" rows={3}
